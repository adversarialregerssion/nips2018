...
class Fcnn(object):
    ...
    def autoencoder(self, input):
        ...
        #     N_bottleneck = 512
        #
        #     fc1 = self.fc_layer(tf.contrib.layers.flatten(out_enc), 8*8*16, N_bottleneck, lname='1', act='relu')
        ...
        with tf.name_scope('decoder'):
            dec1 = self.convlayer(out_enc, 3, 8, 32, strides=[1, 1, 1, 1], lname='L1')
            # dec1 = self.convlayer(out_fc, 3, N_bottleneck, 16, strides=[1, 1, 1, 1], lname='L1')  # (8 x 8)
            # dec2 = self.upsample(dec1, outsize=[8, 8], lname='L1b')  # (24 x 24)
            # dec3 = self.convlayer(dec2, 3, 8, 32, strides=[1, 1, 1, 1], lname='L2')  # (16 x 16)
            dec4 = self.upsample(dec1, outsize=[32, 32], lname='L2b')  # (48 x 48)
